...
`endif
  ...
  always_comb
    for(int i = 0; i < 2; i++) begin
      ...
      op2[i] = rvfi_rs2_rdata[15:0];
      res[i] = ($signed(op1[i]) > $signed(op2[i])) ? (op1[i]) : (op2[i]);
    end
  wire [31:0] result = {res[1], res[0]};
  // ATTENTION!! The core does not check if rvfi_insn[25] = 0!!!
  // This leads to it executing illegal instructions!!!
  // Correct spec_valid: rvfi_valid && !insn_padding && insn_funct5 == 5'd6 && insn_funct1 == 1'b0 && insn_funct3 == 3'b100 && insn_opcode == 7'b101_0111 && rvfi_insn[25] == 1'b0
  assign spec_valid = rvfi_valid && !insn_padding && insn_funct5 == 5'd6 && insn_funct1 == 1'b0 && insn_funct3 == 3'b100 && insn_opcode == 7'b101_0111;
