...
`endif
  ...
  // CV_CLIPR instruction
  reg signed [31:0] compl;
  reg signed [31:0] compg;
  always_comb begin
    begin
      compl = -({1'b0, rvfi_rs2_rdata[30:0]} + 1);
      compg = {1'b0, rvfi_rs2_rdata[30:0]};
    end
  end

  reg [31:0] result;
  always_comb begin
    if ($signed(rvfi_rs1_rdata) <= compl)
      result = compl;
    else if ($signed(rvfi_rs1_rdata) >= compg)
      result = compg;
    else
      result = rvfi_rs1_rdata;
  end
  assign spec_valid = rvfi_valid && !insn_padding && insn_funct7 == 7'b011_1010 && insn_funct3 == 3'b011 && insn_opcode == 7'b010_1011;
