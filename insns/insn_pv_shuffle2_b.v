...
module rvfi_insn_pv_shuffle2_b (
  input                                 rvfi_valid,
...
`endif
  ...
  // PV_SHUFFLE2_B instruction
  reg  [ 2:0] pos;
  reg  [31:0] result;
  always_comb
    for (int i = 0; i < 4; i++) begin
      pos = rvfi_rs2_rdata[i*8 +: 2];
      if (rvfi_rs2_rdata[i*8 + 2])
      result[i*8 +: 8] = rvfi_rs1_rdata[pos*8 +: 8];
      else
      result[i*8 +: 8] = rvfi_rs3_rdata[pos*8 +: 8];
    end
  // ATTENTION!! The core does not check if rvfi_insn[25] = 0!!!
  // This leads to it executing illegal instructions!!!
  assign spec_valid = rvfi_valid && !insn_padding && insn_funct5 == 5'd25 && insn_funct1 == 1'b0 && insn_funct3 == 3'b001 && insn_opcode == 7'b101_0111;
