...
`endif
  ...
  // PV_PACKHI_B instruction
  int pos;
  reg  [31:0] result;
  always_comb begin
      result = rvfi_rs3_rdata;
      pos = 16;
      result[pos +: 8] = rvfi_rs2_rdata[0 +: 8];
      result[(8+pos) +: 8] = rvfi_rs1_rdata[0 +: 8];
  end
  // ATTENTION!! The core does not check if rvfi_insn[25] = 0!!!
  // This leads to it executing illegal instructions!!!
  assign spec_valid = rvfi_valid && !insn_padding && insn_funct5 == 5'd27 && insn_funct1 == 1'b0 && insn_funct3 == 3'b001 && insn_opcode == 7'b101_0111;
