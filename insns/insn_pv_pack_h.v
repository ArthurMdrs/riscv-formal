...
`endif
  ...
  // PV_PACK_H instruction
  int pos;
  reg  [31:0] result;
  always_comb begin
      // ATTENTION!! According to the docs, pos = 0!!!
      // In newer docs: rvfi_insn[25]==0 means pv.pack and rvfi_insn[25]==1 means pv.pack.h
      pos = (rvfi_insn[25]) ? (16) : (0);
      result[ 0 +: 16] = rvfi_rs2_rdata[pos +: 16];
      result[16 +: 16] = rvfi_rs1_rdata[pos +: 16];
  end
  // ATTENTION!! The core does not check if rvfi_insn[25] = 0!!!
  // This leads to it executing illegal instructions!!!
  assign spec_valid = rvfi_valid && !insn_padding && insn_funct5 == 5'd26 && insn_funct1 == 1'b0 && insn_funct3 == 3'b000 && insn_opcode == 7'b101_0111;
