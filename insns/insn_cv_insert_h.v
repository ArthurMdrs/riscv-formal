...
`endif
  ...
  // CV_INSERT_H instruction
  wire        sel = rvfi_insn[25];
  reg  [15:0] res [1:0];
  always_comb
    for(int i = 0; i < 2; i++) begin
      if (i == sel)
        res[i] = rvfi_rs1_rdata[15:0];
      else
        res[i] = rvfi_rs3_rdata[i*16+:16];
    end
  wire [31:0] result = {res[1], res[0]};
  wire flag = rvfi_insn[24:20] == '0;
  assign spec_valid = rvfi_valid && !insn_padding && flag && insn_funct5 == 5'b1_0111 && insn_funct1 == 1'b0 && insn_funct3 == 3'b100 && insn_opcode == 7'b111_1011;
