...
module rvfi_insn_pv_shuffleI0_sci_b (
  input                                 rvfi_valid,
...
`endif
  ...
  // PV_SHUFFLEI0_SCI_B instruction
  wire [5:0] insn_imm = {rvfi_insn[24:20], rvfi_insn[25]};
  reg  [ 2:0] pos;
  reg  [31:0] result;
  always_comb
    for (int i = 0; i < 4; i++) begin
      pos = (i == 3) ? (0) : (insn_imm[i*2 +: 2]);
      result[i*8 +: 8] = rvfi_rs1_rdata[pos*8 +: 8];
    end
  assign spec_valid = rvfi_valid && !insn_padding && insn_funct5 == 5'd24 && insn_funct1 == 1'b0 && insn_funct3 == 3'b111 && insn_opcode == 7'b101_0111;
