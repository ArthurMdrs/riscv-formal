...
module rvfi_insn_cv_sdotusp_sci_b (
  input                                 rvfi_valid,
...
`endif
  ...
  // CV_SDOTUSP_SCI_B instruction
`ifdef RISCV_FORMAL_ALTOPS
  wire [31:0] op2 = {4{{2{rvfi_insn[24]}}, rvfi_insn[24:20], rvfi_insn[25]}};
...
`else
  ...
  always_comb
    for(int i = 0; i < 4; i++) begin
      ...
      res[i] = $signed({1'b0, op1[i]}) * $signed(op2[i]);
    end
  wire [31:0] res_sum = $signed(res[0]) + $signed(res[1]) + $signed(res[2]) + $signed(res[3]);
  wire [31:0] result = res_sum + rvfi_rs3_rdata;
`endif
  wire flag = 1'b1;
  assign spec_valid = rvfi_valid && !insn_padding && flag && insn_funct5 == 5'b1_0100 && insn_funct1 == 1'b0 && insn_funct3 == 3'b111 && insn_opcode == 7'b111_1011;
