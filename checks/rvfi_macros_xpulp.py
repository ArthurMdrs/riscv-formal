...
#### ================   XPULP - BEGIN   ================ ####
group_xpulp = Group(
    condition="RISCV_FORMAL_CUSTOM_ISA",
    name="rvformal_xpulp",
    channels="`RISCV_FORMAL_NRET",
    signals=[
        ("5", "rs3_addr"),
        ("`RISCV_FORMAL_XLEN", "rs3_rdata"),
        ("5", "post_rd_addr"),
        ("`RISCV_FORMAL_XLEN", "post_rd_wdata"),
        ("1", "is_hwlp"),
        ("`RISCV_FORMAL_XLEN", "hwlp_start")
    ]
).print_macros()
#### ================   XPULP - END   ================ ####
...
rvfi = Group(
    ...
    append = [group_extamo, group_rollback, group_fault, *csr_groups, custom_csr, group_xpulp]
).print_macros()
